...
public class CoxCMAutoSplitter extends Plugin {
  ...
  Robot r = new Robot(); // For sending event to live splits
  ...
  public CoxCMAutoSplitter() throws AWTException {
  }
  ...
  private void send_split() {
    r.keyPress(config.key().getKeyCode());
    r.keyRelease(config.key().getKeyCode());
  }
